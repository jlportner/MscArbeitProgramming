...
def claimf(k):
    ...
    for n in range(k):
        ...
        addToDict(C,"0c" + "0" * n + "1" + "0" * m, (-1)**m * binom(k, m))
    return C | {"1z" + "0" * k: 1, "1c" + "0" * k: (-1)**k}
...
def propFTil(C):
    ...
    for el, coef in C:
        ...
        if z == "z":
            ...
            word = el
            addToDict(result, word + "0", coef)
            addToDict(result, "0" + word, -coef)
    return result
...
def claimfTil(k):
    ...
    for n in range(0,k):
        m = k-1-n
        result[n*"0" + "1" + m * "0"] = binom(n+m,n) * (-1)**n * (2*k-1)
    return result
